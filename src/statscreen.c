...
// negative bars palette
#define RGB2HEX(r, g, b) RGB(r/8, g/8, b/8)
static u16 CONST_DATA mss_negative_pals[] = {
    RGB2HEX(0, 0, 0),
...
static void ResetTextPalInfo()
{
	gCurrentFont->unk10 = gCurrentFont->unk10 & 0x0FFF;
}

static void DrawTextInline_WithNegPal(struct TextHandle *th, 
                               u16 *dest, 
                               int color_id, 
                               int x, 
                               int tile_width, 
                               const char *string)
{
    ...
    /* Note only ally needs display growth! */
    if (FACTION_BLUE != UNIT_FACTION(gStatScreen.unit))
        color_id = TEXT_COLOR_GOLD;
    ...
    DrawTextInline(th, dest, color_id, x, tile_width, string);
    ResetTextPalInfo();
}

static int GrowthToTextPal(int growth)
{
    if (growth > 90)
        growth = 90;
    
    return 9 - growth/10 + 5;
}

struct NewSSTextDispInfo
{
    struct TextHandle* text;
    u16* tilemap;
    int (*growth_getter)(struct Unit* unit);
    u8 xoff;
    const unsigned* mid;
};

static
struct NewSSTextDispInfo const sPage0TextInfoNew[] = {
    { gStatScreen.text + STATSCREEN_TEXT_SKLLABEL,   gBmFrameTmap0 + TILEMAP_INDEX(1, 3),  SklGrowthGetter, 0, &gMid_Skl },
    { gStatScreen.text + STATSCREEN_TEXT_SPDLABEL,   gBmFrameTmap0 + TILEMAP_INDEX(1, 5),  SpdGrowthGetter, 0, &gMid_Spd },
    { gStatScreen.text + STATSCREEN_TEXT_LCKLABEL,   gBmFrameTmap0 + TILEMAP_INDEX(1, 7),  LckGrowthGetter, 0, &gMid_Lck },
    { gStatScreen.text + STATSCREEN_TEXT_DEFLABEL,   gBmFrameTmap0 + TILEMAP_INDEX(1, 9),  DefGrowthGetter, 0, &gMid_Def },
    { gStatScreen.text + STATSCREEN_TEXT_RESLABEL,   gBmFrameTmap0 + TILEMAP_INDEX(1, 11), ResGrowthGetter, 0, &gMid_Res },

    { }, // end
};

static
void DisplayTextsWithGrowthPal(const struct NewSSTextDispInfo* infos)
{
    while (infos->text)
    {
        DrawTextInline_WithNegPal(
            infos->text,
            infos->tilemap,
            GrowthToTextPal(infos->growth_getter(gStatScreen.unit)),
            infos->xoff, 0,
...
{
    #define MSG_INDEX_SKILLS 0x0D4F
    #define MSG_INDEX_NONE 0x0D50

    int i;
    const int X_BASE = 1;
    const int Y_BASE = 13;
    struct skill_fast_list *list = get_or_create_skill_fast_list(gStatScreen.unit);
	...
	Text_AppendString(text, GetStringFromIndex(MSG_INDEX_SKILLS));

    if (0 == list->len) {
		Text_SetColorId(text, TEXT_COLOR_GRAY);
		Text_SetXCursor(text, 32);
		Text_AppendString(text, GetStringFromIndex(MSG_INDEX_NONE) );
		Text_Draw(text, gBmFrameTmap0 + TILEMAP_INDEX(X_BASE, Y_BASE));
	}
    else {
        Text_Draw(text, gBmFrameTmap0 + TILEMAP_INDEX(X_BASE, Y_BASE));
        ...
        for (i = 0; i < list->len; ++i)
            DrawIcon(
				gBmFrameTmap0 + TILEMAP_INDEX(
					X_BASE + 2 * (i % 5) + 3, 
					Y_BASE + 2 * (i / 5)), 
				SKILL_ICON(list->skill[i]), 
...
void DisplayPage0(void)
{
    ...
    DisplayTextsWithGrowthPal(sPage0TextInfoNew);
    ...
    if (UnitHasMagicRank(gStatScreen.unit))
    {
        ...
        DrawTextInline_WithNegPal(
            ...
            #ifdef CONFIG_STR_MAG_SPLIT
            GrowthToTextPal(MagGrowthGetter(gStatScreen.unit)),
            #else
            GrowthToTextPal(PowGrowthGetter(gStatScreen.unit)),
            #endif /* CONFIG_STR_MAG_SPLIT */
            0, 0,
...
void StatScreen_InitDisplay(struct Proc* proc)
{
    ...
    // Preppare for negative palettes
    ApplyPalettes(mss_negative_pals, STATSCREEN_BGPAL_NEG, 2);
...
static void HbRedirect_Skill(struct HelpBoxProc* proc) {
	...
	if (proc->info->mid < get_or_create_skill_fast_list(gStatScreen.unit)->len)
		return;
...
static void HbPopuplate_Skill(struct HelpBoxProc* proc){
	...
	int skill_id = get_or_create_skill_fast_list(gStatScreen.unit)->skill[proc->info->mid];

	proc->mid = get_skill_desc_msg(skill_id);
...
static DECL_INFO sHelpInfo_Skill0;
static DECL_INFO sHelpInfo_Skill1;
static DECL_INFO sHelpInfo_Skill2;
static DECL_INFO sHelpInfo_Skill3;
static DECL_INFO sHelpInfo_Skill4;
static DECL_INFO sHelpInfo_Skill5;
static DECL_INFO sHelpInfo_Skill6;
static DECL_INFO sHelpInfo_Skill7;
static DECL_INFO sHelpInfo_Skill8;
static DECL_INFO sHelpInfo_Skill9;
...
static DECL_INFO sHelpInfo_Skill0 =
{
    &sHelpInfo_08A00C50, &sHelpInfo_Skill5, &sHelpInfo_08A00B8C, &sHelpInfo_Skill1,
    0x80, 0x78, 0, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill1 =
{
    &sHelpInfo_08A00C50, &sHelpInfo_Skill6, &sHelpInfo_Skill0, &sHelpInfo_Skill2,
    0x90, 0x78, 1, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill2 =
{
    &sHelpInfo_08A00C50, &sHelpInfo_Skill7, &sHelpInfo_Skill1, &sHelpInfo_Skill3,
    0xA0, 0x78, 2, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill3 =
{
    &sHelpInfo_08A00C50, &sHelpInfo_Skill8, &sHelpInfo_Skill2, &sHelpInfo_Skill4,
    0xB0, 0x78, 3, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill4 =
{
    &sHelpInfo_08A00C50, &sHelpInfo_Skill9, &sHelpInfo_Skill3, NULL,
    0xC0, 0x78, 4, HbRedirect_Skill, HbPopuplate_Skill,
};

static DECL_INFO sHelpInfo_Skill5 =
{
    &sHelpInfo_Skill0, NULL, &sHelpInfo_08A00BA8, &sHelpInfo_Skill6,
    0x80, 0x88, 5, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill6 =
{
    &sHelpInfo_Skill1, NULL, &sHelpInfo_Skill5, &sHelpInfo_Skill7,
    0x90, 0x88, 6, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill7 =
{
    &sHelpInfo_Skill2, NULL, &sHelpInfo_Skill6, &sHelpInfo_Skill8,
    0xA0, 0x88, 7, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill8 =
{
    &sHelpInfo_Skill3, NULL, &sHelpInfo_Skill7, &sHelpInfo_Skill9,
    0xB0, 0x88, 8, HbRedirect_Skill, HbPopuplate_Skill,
};
static DECL_INFO sHelpInfo_Skill9 =
{
    &sHelpInfo_Skill4, NULL, &sHelpInfo_Skill8, NULL,
    0xC0, 0x88, 9, HbRedirect_Skill, HbPopuplate_Skill,
};
...
static DECL_INFO sHelpInfo_08A00B8C =
{
    &sHelpInfo_08A00B54, &sHelpInfo_08A00BA8, &sHelpInfo_08A00B70, &sHelpInfo_Skill0,
    38, 120, 0x543,
};

static DECL_INFO sHelpInfo_08A00BA8 =
{
    &sHelpInfo_08A00B70, NULL, NULL, &sHelpInfo_Skill5,
    6, 136, 0x544,
};
...
static DECL_INFO sHelpInfo_08A00C50 =
{
    &sHelpInfo_08A00C34, &sHelpInfo_Skill0, &sHelpInfo_08A00B54, &sHelpInfo_08A00CF8,
    102, 104, 0x54C,
};
...
static DECL_INFO sHelpInfo_08A00CF8 =
{
    &sHelpInfo_08A00CDC, &sHelpInfo_Skill4, &sHelpInfo_08A00C50, NULL,
