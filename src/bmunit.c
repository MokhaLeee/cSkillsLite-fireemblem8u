...
inline int GetUnitMaxHp(struct Unit* unit) {
    int stat = unit->maxHP;
    stat += GetItemHpBonus((u16) GetUnitEquippedWeapon(unit));
...
inline int GetUnitPower(struct Unit* unit) {
    int stat = unit->pow;
    stat += COND_ADD(TRUE, GetItemPowBonus((u16) GetUnitEquippedWeapon(unit)));
    stat += COND_ADD(skill_tester(unit, skill_index_unit_bonus_pow), 2);
    return MINUS_ZERO(stat);
}

inline int GetUnitSkill(struct Unit* unit) {
    int stat = (unit->state & US_RESCUING)
              ? unit->skl / 2
              : unit->skl;
    stat += COND_ADD(TRUE, GetItemSklBonus((u16) GetUnitEquippedWeapon(unit)));
    return MINUS_ZERO(stat);
}
...
inline int GetUnitSpeed(struct Unit* unit) {
    int stat = (unit->state & US_RESCUING)
              ? unit->spd / 2
              : unit->spd;
    stat += COND_ADD(TRUE, GetItemSpdBonus((u16) GetUnitEquippedWeapon(unit)));
    return MINUS_ZERO(stat);
}

inline int GetUnitDefense(struct Unit* unit) {
    int stat = unit->def;
    stat += COND_ADD(TRUE, GetItemDefBonus((u16) GetUnitEquippedWeapon(unit)));
    return MINUS_ZERO(stat);
}

inline int GetUnitResistance(struct Unit* unit) {
    int stat = unit->res;
    stat += COND_ADD(TRUE, GetItemResBonus((u16) GetUnitEquippedWeapon(unit)));
    stat += COND_ADD(TRUE, unit->barrierDuration);
    return MINUS_ZERO(stat);
}

inline int GetUnitLuck(struct Unit* unit) {
    int stat = unit->lck;
    stat += COND_ADD(TRUE, GetItemLckBonus((u16) GetUnitEquippedWeapon(unit)));
    return MINUS_ZERO(stat);
}
