 ...
 */
static void PreBC_BlowSkills(struct BattleUnit* attacker, struct BattleUnit* defender)
{
    int cond;

    cond = skill_tester(&attacker->unit, skill_index_battle_status_blow_death);
    attacker->battleAttack += COND_ADD(cond, 6);
}

static void PreBC_Terminal(struct BattleUnit* attacker)
{
    attacker->battleAttack      = MINUS_ZERO(attacker->battleAttack);
    attacker->battleDefense     = MINUS_ZERO(attacker->battleDefense);
    attacker->battleHitRate     = MINUS_ZERO(attacker->battleHitRate);
    attacker->battleAvoidRate   = MINUS_ZERO(attacker->battleAvoidRate);
    attacker->battleCritRate    = MINUS_ZERO(attacker->battleCritRate);
    attacker->battleDodgeRate   = MINUS_ZERO(attacker->battleDodgeRate);
}
...
void ComputeBattleUnitStats(struct BattleUnit* attacker, struct BattleUnit* defender)
{
    ...
    PreBC_BlowSkills(attacker, defender);

    /* do not forget minus-zero */
    PreBC_Terminal(attacker);
}
...
int MLU_SetBuStatUp(struct BattleUnit* bu, int bonus){
	bu->changeHP  = GetStatIncrease( HpGrowthGetter(&bu->unit)  + bonus);
	bu->changePow = GetStatIncrease( PowGrowthGetter(&bu->unit) + bonus);
	bu->changeSkl = GetStatIncrease( SklGrowthGetter(&bu->unit) + bonus);
	bu->changeSpd = GetStatIncrease( SpdGrowthGetter(&bu->unit) + bonus);
	bu->changeDef = GetStatIncrease( DefGrowthGetter(&bu->unit) + bonus);
	bu->changeRes = GetStatIncrease( ResGrowthGetter(&bu->unit) + bonus);
	bu->changeLck = GetStatIncrease( LckGrowthGetter(&bu->unit) + bonus);
...
void CheckBattleUnitLevelUp(struct BattleUnit* bu) {
    if (CanBattleUnitGainLevels(bu) && bu->unit.exp >= 100) {
        ...
        statGainTotal = MLU_SetBuStatUp(bu, growthBonus);

        if (statGainTotal == 0)
            MLU_SetBuStatUp(bu, growthBonus);
